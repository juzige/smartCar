...
{
    // 开启模块时钟
    SIM->SCGC5 |= SIM_SCGC5_LPTIMER_MASK;                   //使能LPT模块时钟
    
    //设置输入管脚    
    if(altn == lpt0_alt1)       port_init(A19, ALT6 | PULLUP);      //在A19上使用 ALT6
    else if(altn == lpt0_alt2)  port_init(C5, ALT4 | PULLUP);       //在C5上使用 ALT4
    ...
    LPTMR0->PSR  =  ( 0
                      ...
                      //| LPTMR_PSR_PRESCALE(0)               //预分频值 = 2^(n+1) ,n = 0~ 0xF
                     );
...
uint16 lptmr_pulse_get(void)
{
    ...
    {
#ifdef MK60DN
        LPTMR0->CNR = 0; //按照数据手册要求先写任意值然后在读取
#endif
        data = LPTMR0->CNR;
